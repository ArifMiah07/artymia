    ...
    <div className="p-4">
            ...
            <div className="absolute inset-0 bg-white bg-opacity-80 transform translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-in-out">
              <div className="flex flex-col justify-center items-center h-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out">
                ...
                <div onClick={() => openModal(img)}>
                  <button
                    className="bg-white text-black hover:text-white border hover:bg-gray-600 p-3 "
      ...
      {selectedImage && (
        <dialog id="image_modal" className="modal">
          <div className="modal-box bg-white w-11/12 max-w-5xl flex flex-col items-center ">
            <form
              onClick={closeModal}
              className=" modal-action"
              method="dialog">
              {/* if there is a button in form, it will close the modal */}
              <button className="btn btn-sm md:btn-md lg:btn-lg btn-circle btn-ghost absolute right-2 top-2">
                ✕
              </button>
            </form>
